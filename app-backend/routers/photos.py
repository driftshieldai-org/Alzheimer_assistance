...
async def upload_photo(
  ...
  photoDate: str = Form(...),
  latitude: Optional[str] = Form(None), # <-- Optional latitude
  longitude: Optional[str] = Form(None), # <-- Optional longitude
  ...
  # 2️⃣ Generate detailed visual description with Gemini
  try:
  ...
  # 3️⃣ Save to Firestore (Including optional Location block)
  try:
    ...
    doc_data = {
      ...
      "geminiDescription": gemini_description,
      "photoDate": photoDate,
      "imageUrl": filename,
      "filename": filename,
    ...
    # Only append location data if coordinates were passed from the client
